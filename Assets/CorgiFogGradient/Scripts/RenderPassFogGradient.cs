    ...
    public class RenderPassFogGradient : ScriptableRenderPass
    {
        ...
        private void UpdateGradientTexture(RenderFeatureFogGradient.FogGradientSettings settings)
        {
            ...
            var job = new GenerateFogTextureJob()
            {
                ...
                distanceGradientColorKeys = new NativeArray<GradientColorKey>(distanceGradient.colorKeys, Allocator.TempJob),
                distanceGradientAlphaKeys = new NativeArray<GradientAlphaKey>(distanceGradient.alphaKeys, Allocator.TempJob),

                heightGradientColorKeys = new NativeArray<GradientColorKey>(heightGradient.colorKeys, Allocator.TempJob),
                heightGradientAlphaKeys = new NativeArray<GradientAlphaKey>(heightGradient.alphaKeys, Allocator.TempJob),
            };

            var handle = job.Schedule(volume, settings.quality);
                handle.Complete();

            job.distanceGradientColorKeys.Dispose();
            job.distanceGradientAlphaKeys.Dispose();

            job.heightGradientColorKeys.Dispose();
            job.heightGradientAlphaKeys.Dispose();
        ...
        private struct GenerateFogTextureJob : IJobParallelFor
        {
            ...
            [ReadOnly] public NativeArray<GradientColorKey> distanceGradientColorKeys;
            [ReadOnly] public NativeArray<GradientAlphaKey> distanceGradientAlphaKeys;

            [ReadOnly] public NativeArray<GradientColorKey> heightGradientColorKeys;
            [ReadOnly] public NativeArray<GradientAlphaKey> heightGradientAlphaKeys;
            ...
            private Color EvaluateGradientColor(float t, NativeArray<GradientColorKey> keys)
            {
                if (t <= keys[0].time)
                {
                    return keys[0].color;
            ...
            private float EvaluateGradientAlpha(float t, NativeArray<GradientAlphaKey> keys)
            {
                if (t <= keys[0].time)
                {
                    return keys[0].alpha;
